...
class Object
  def value_as_handler_action(an_exception_handler)
...
class PassToNextHandlerAction
  ...
  def value_as_handler_action(an_exception_handler)
    an_exception_handler.pass_to_next_handler @an_exception
  end
end

class RetryBlockAction
  def value_as_handler_action(an_exception_handler)
    an_exception_handler.retry
  end
end

class ResumeAction
  def initialize(an_object)
    @object_to_resume_with = an_object
  end
...
class DefinedExceptionHandler < ExceptionHandler
  ...
  def pass_to_next_handler(an_exception)
    @return_continuation.call @previous.handle an_exception
  end
  ...
  def self.install_new_handler_while_evaluating(a_block,an_exception_class,handler,return_closure)
    ...
    self.last_handler = self.new an_exception_class,handler,return_closure,self.last_handler,retry_token
    begin
      begin
        result = a_block.call
  ...
  def handle(an_exception)
    if should_handle? an_exception
      handler_action = @handler.call an_exception
      handler_action.value_as_handler_action self
    else
      pass_to_next_handler an_exception
    end
  end
  ...
  def initialize(a_condition,handler,return_closure,previous,retry_token)
    @handler = handler
...
class ExceptionImplementationTest < Test::Unit::TestCase
  ...
  def test_7
    ...
    outher_exception_handler_evaluated = false
    ...
    lambda {
      ...
      outher_exception_handler_evaluated = true
    end

    assert inner_exception_handler_evaluated
    assert outher_exception_handler_evaluated
  ...
  def test_9
    result = lambda {
      NewException.throw + 1 }.call_handling NewException do |an_exception |
